...
### Shared `_targets` store

Use `aml_init()` instead of a manual `az ml datastore mount` + symlink
step. It mounts blob storage, establishes parity symlinks, and pulls
cloud metadata into your local session in one call. See [Sharing the
`_targets`
store](https://jeffery-leirness.github.io/amltargets/articles/amltargets-run.html#sharing-the-_targets-store-between-local-and-cluster)
in the vignette for the full workflow.
...
`tar_aml_job()` writes results into the blob-backed `_targets/` cache on
the cluster. The `aml_*()` store helpers keep your local session in sync
with it.

`aml_init()` replaces any manual mount step — it detects the
environment, mounts blob storage, and pulls cloud metadata locally. Call
it once at the top of your session, then push metadata back after each
`tar_make()`:
...
tar_make()
aml_store_push(store$cloud_store_base, project = "region_a")
```

After cluster jobs land, call `aml_store_sync()` to remount and re-pull
fresh metadata. `aml_store_verify()` self-heals a broken local cache;
`aml_store_reset()` wipes it cleanly without touching cloud objects.

See the [store lifecycle vignette
section](https://jeffery-leirness.github.io/amltargets/articles/amltargets-run.html#sharing-the-_targets-store-between-local-and-cluster)
for full details.
